...
export function Profile() {
  ...
  if (!isLoggedIn || loading) {
    return (
      <div className="app">
        <PageTitleBar title={isEnUi ? 'Profile' : '个人中心'} backTo="/" showLangToggle />
      ...
      <div style={{ padding: '0 16px' }}>
        <div className="card profile-info-card" style={{ padding: 18, marginTop: 12, marginBottom: 12 }}>
          <div className="profile-row">
            <span className="profile-label">{isEnUi ? 'Nickname' : '昵称'}</span>
            <span className="profile-value">{nickname}</span>
          </div>
          <div className="profile-row">
            <span className="profile-label">{isEnUi ? 'Phone' : '手机号'}</span>
